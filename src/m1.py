...
def broken_3(n, point, length, distance_between_lines, window):
    ...
    for i in range(n):
        a = rg.Point(point.x+distance_between_lines*i, point.y)
        b = rg.Point(point.x+distance_between_lines*i, point.y + length)
        line = rg.Line(a, b)
...
def broken_7(n, point, length, distance_between_lines, window):
    ...
    for i in range(n):
        left = rg.Point(left.x, left.y + c)
        right = rg.Point(right.x, right.y + c)
